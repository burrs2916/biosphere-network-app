<script lang="ts">
	...
	interface CommandInjectionVuln {
		parameter: string;
	...
	interface CommandInjectionSafeEntry {
		parameter: string;
		tests_run: number;
		method: string;
	}

	interface CommandInjectionErrorEntry {
		parameter: string;
		payload: string;
		error: string;
		method: string;
	...
	interface OsTypeDistribution {
		os_type: string;
		count: number;
		vulnerable_count: number;
	}

	interface CommandInjectionResult {
		url: string;
		vulnerabilities: CommandInjectionVuln[];
		safe_parameters: CommandInjectionSafeEntry[];
		errors: CommandInjectionErrorEntry[];
		tests_performed: number;
		parameters_tested: string[];
		scan_duration_ms: number;
		summary: string;
		os_type_distribution: OsTypeDistribution[];
	}
	...
	let result: CommandInjectionResult | null = $state(null);
	let error = $state('');
	...
	const TOOL_NAME = 'command_injection';
	let historyComponent = $state<ToolHistory>();

	function getScanLevelLabel(level: string): string {
		const labels: Record<string, string> = {
			basic: $tr('commandInjection.levelBasic'),
			moderate: $tr('commandInjection.levelModerate'),
			aggressive: $tr('commandInjection.levelAggressive')
		};
	...
	function getOsTypeColor(osType: string): string {
		switch (osType) {
			case 'linux': return '#3b82f6';
			case 'windows': return '#a855f7';
	...
	function getOsTypeIcon(osType: string): string {
		switch (osType) {
			case 'linux': return '🐧';
			case 'windows': return '🪟';
			case 'mixed': return '🔄';
	...
	function getInjectionTypeIcon(injType: string): string {
		switch (injType) {
			case 'chain': return '🔗';
			case 'pipe': return '📤';
			case 'substitution': return '🔄';
			case 'newline': return '↩️';
	...
	function getFilteredVulns() {
		return result?.vulnerabilities.filter(v => {
			...
			if (typeFilter !== 'all' && v.injection_type !== typeFilter) return false;
			if (osFilter !== 'all' && v.os_type !== osFilter) return false;
			...
			if (searchQuery) {
				...
				return v.parameter.toLowerCase().includes(q) ||
					...
					v.injection_type.toLowerCase().includes(q) ||
	...
	function getUniqueTypes() {
		return [...new Set(result?.vulnerabilities.map(v => v.injection_type) || [])];
	}

	function getUniqueOsTypes() {
		return [...new Set(result?.vulnerabilities.map(v => v.os_type) || [])];
	}
	...
	function getTypeChartData() {
		...
		const maxVulns = Math.max(...types.map(t => result!.vulnerabilities.filter(v => v.injection_type === t).length));
		return types.map(t => ({
			injection_type: t,
			count: result!.vulnerabilities.filter(v => v.injection_type === t).length,
			percent: maxVulns > 0 ? (result!.vulnerabilities.filter(v => v.injection_type === t).length / maxVulns) * 100 : 0
		}));
	}

	async function scan() {
		if (!url.trim()) {
			error = $tr('commandInjection.urlRequired');
			return;
		}
		if (!testGet && !testPost && !testCookies && !testHeaders) {
			error = $tr('commandInjection.methodRequired');
			return;
		}
		...
		try {
			...
			result = await invoke<CommandInjectionResult>('scan_command_injection_command', {
				config: {
					url: url.trim(),
					timeout,
					threads,
	...
	async function exportResult() {
		...
		try {
			...
			const path = await save({
				defaultPath: `cmdi_scan_${Date.now()}.${exportFormat}`,
			...
			if (exportFormat === 'json') {
				...
				const headers = ['Parameter', 'Injection Type', 'OS Type', 'Severity', 'Payload', 'Evidence', 'Confidence', 'Method', 'Status', 'Response Time'];
				const rows = result.vulnerabilities.map(v =>
					[v.parameter, v.injection_type, v.os_type, v.severity, v.payload, v.evidence, v.confidence.toString(), v.method, (v.http_status || '').toString(), (v.response_time_ms || '').toString()]
				);
...
<div class="nd-page">
	<div class="page-header">
		<div class="header-left">
			...
			<h1 class="page-title">💉 {$tr('commandInjection.title')}</h1>
			<p class="page-subtitle">{$tr('commandInjection.subtitle')}</p>
		</div>
	</div>

	<div class="tabs">
		<button class="tab-btn {activeMainTab === 'analyze' ? 'active' : ''}" onclick={() => activeMainTab = 'analyze'}>
			<span class="tab-icon">🔍</span> {$tr('commandInjection.scan')}
		</button>
		<button class="tab-btn {activeMainTab === 'history' ? 'active' : ''}" onclick={() => activeMainTab = 'history'}>
			<span class="tab-icon">📋</span> {$tr('commandInjection.history')}
		</button>
		<button class="tab-btn {activeMainTab === 'help' ? 'active' : ''}" onclick={() => activeMainTab = 'help'}>
			<span class="tab-icon">📖</span> {$tr('commandInjection.help')}
		</button>
	</div>
		...
		<div class="content-grid">
			<div class="input-section">
				<div class="section-card">
					<h2 class="section-title">{$tr('commandInjection.configTitle')}</h2>
					<p class="section-desc">{$tr('commandInjection.configDesc')}</p>

					<div class="form-group">
						<label class="form-label">{$tr('commandInjection.targetUrl')}</label>
					...
					<div class="form-group">
						<label class="form-label">{$tr('commandInjection.scanLevel')}</label>
						<div class="mode-grid">
					...
					<div class="form-row">
						<div class="form-group">
							<label class="form-label">⏱️ {$tr('commandInjection.timeout')}</label>
							<input type="number" bind:value={timeout} class="form-input" min="5" max="60" disabled={processing} />
						</div>
						<div class="form-group">
							<label class="form-label">🔀 {$tr('commandInjection.threads')}</label>
							<input type="number" bind:value={threads} class="form-input" min="1" max="20" disabled={processing} />
						</div>
					</div>

					<div class="form-group">
						<label class="form-label">{$tr('commandInjection.testMethods')}</label>
					...
					<div class="form-group">
						<label class="form-label">🎯 {$tr('commandInjection.customParams')}</label>
						<textarea bind:value={customParams} placeholder="id, cmd, exec, file..." class="form-input" style="height: 60px; resize: vertical;" disabled={processing}></textarea>
					</div>

					<div class="button-group">
						<button class="btn-primary" onclick={scan} disabled={processing || !url.trim() || (!testGet && !testPost && !testCookies && !testHeaders)}>
							{#if processing}<span class="spinner"></span>{$tr('commandInjection.scanning')}{:else}💉 {$tr('commandInjection.startScan')}{/if}
						</button>
					...
					{:else if result}
						<div class="result-header">
							...
							<div class="header-actions">
								<div class="resource-score-badge">
									...
									<span class="score-label">{$tr('commandInjection.vulnsFound')}</span>
								</div>
						...
						<div class="result-tabs">
							<button class="result-tab {activeResultTab === 'overview' ? 'active' : ''}" onclick={() => activeResultTab = 'overview'}>
								📊 {$tr('commandInjection.tabOverview')}
							</button>
							<button class="result-tab {activeResultTab === 'vulns' ? 'active' : ''}" onclick={() => activeResultTab = 'vulns'}>
								🔴 {$tr('commandInjection.tabVulns')} ({result.vulnerabilities.length})
							</button>
							<button class="result-tab {activeResultTab === 'safe' ? 'active' : ''}" onclick={() => activeResultTab = 'safe'}>
								🟢 {$tr('commandInjection.tabSafe')} ({result.safe_parameters.length})
							</button>
							{#if result.errors.length > 0}
								<button class="result-tab {activeResultTab === 'errors' ? 'active' : ''}" onclick={() => activeResultTab = 'errors'}>
									❌ {$tr('commandInjection.tabErrors')} ({result.errors.length})
								</button>
							{/if}
							{#if result.os_type_distribution.length > 0}
								<button class="result-tab {activeResultTab === 'ostype' ? 'active' : ''}" onclick={() => activeResultTab = 'ostype'}>
									💻 {$tr('commandInjection.tabOsType')}
								</button>
							{/if}
						</div>
						...
						{#if activeResultTab === 'overview'}
							<div class="overview-grid">
								<div class="overview-stat">
									<span class="stat-label">{$tr('commandInjection.testsPerformed')}</span>
									<span class="stat-value">{result.tests_performed}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">🔴 {$tr('commandInjection.criticalSeverity')}</span>
								...
								<div class="overview-stat">
									<span class="stat-label">🟠 {$tr('commandInjection.highSeverity')}</span>
								...
								<div class="overview-stat">
									<span class="stat-label">🟡 {$tr('commandInjection.mediumSeverity')}</span>
								...
								<div class="overview-stat">
									<span class="stat-label">🎯 {$tr('commandInjection.paramsTested')}</span>
									<span class="stat-value">{result.parameters_tested.length}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">🟢 {$tr('commandInjection.safeParams')}</span>
									<span class="stat-value" style="color: #22c55e">{result.safe_parameters.length}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">⏱️ {$tr('commandInjection.scanDuration')}</span>
									<span class="stat-value">{formatDuration(result.scan_duration_ms)}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">⚠️ {$tr('commandInjection.errorCount')}</span>
							...
							{#if result && result.vulnerabilities.length > 0}
								<div class="category-chart">
									<h3 class="subsection-title">{$tr('commandInjection.injectionTypeChart')}</h3>
									<div class="chart-bars">
										{#each getTypeChartData() as data}
											<div class="chart-row">
												<span class="chart-label">{data.injection_type}</span>
							...
							{#if result && result.parameters_tested.length > 0}
								<div class="params-list">
									<h3 class="subsection-title">{$tr('commandInjection.testedParamsList')}</h3>
									<div class="chip-list">
						...
						{#if activeResultTab === 'vulns'}
							{#if result && result.vulnerabilities.length > 0}
								<div class="filter-bar">
									<input type="text" bind:value={searchQuery} placeholder="{$tr('commandInjection.searchVulns')}..." class="filter-input" />
									<select bind:value={severityFilter} class="filter-select">
										<option value="all">{$tr('commandInjection.allSeverities')}</option>
									...
									<select bind:value={typeFilter} class="filter-select">
										<option value="all">{$tr('commandInjection.allTypes')}</option>
										{#each getUniqueTypes() as t}
											<option value={t}>{getInjectionTypeIcon(t)} {t}</option>
										{/each}
									</select>
									<select bind:value={osFilter} class="filter-select">
										<option value="all">{$tr('commandInjection.allOsTypes')}</option>
										{#each getUniqueOsTypes() as o}
											<option value={o}>{getOsTypeIcon(o)} {o}</option>
										{/each}
									</select>
									<select bind:value={methodFilter} class="filter-select">
										<option value="all">{$tr('commandInjection.allMethods')}</option>
										{#each getUniqueMethods() as m}
											<option value={m}>{getMethodIcon(m)} {m}</option>
										{/each}
									</select>
								</div>
										...
										<div class="vuln-card">
											<div class="vuln-header">
												<div class="vuln-header-left">
													...
													<span class="type-badge" style="background: rgba(168, 85, 247, 0.15); color: #c4b5fd">{getInjectionTypeIcon(vuln.injection_type)} {vuln.injection_type}</span>
													<span class="os-badge" style="background: {getOsTypeColor(vuln.os_type)}20; color: {getOsTypeColor(vuln.os_type)}">{getOsTypeIcon(vuln.os_type)} {vuln.os_type}</span>
												</div>
												<span class="confidence-badge">
													{$tr('commandInjection.confidence')}: {getConfidencePercent(vuln.confidence)}%
												</span>
											</div>
											<div class="vuln-body">
												<div class="vuln-field">
													<span class="field-label">🎯 {$tr('commandInjection.parameter')}</span>
							...
							{:else}
								<div class="safe-state">
									...
									<span class="safe-text">{$tr('commandInjection.noVulns')}</span>
								</div>
							{/if}
						{/if}

						{#if activeResultTab === 'safe'}
							{#if result.safe_parameters.length > 0}
								<div class="safe-list">
									{#each result.safe_parameters as safe}
										<div class="safe-item">
											<span class="safe-check">✅</span>
											<span class="safe-param">{safe.parameter}</span>
											<span class="safe-method">{safe.method}</span>
											<span class="safe-tests">{$tr('commandInjection.testsRun')}: {safe.tests_run}</span>
										</div>
									{/each}
								</div>
							{:else}
								<div class="empty-state">{$tr('commandInjection.noSafeParams')}</div>
							{/if}
						{/if}
									...
									{#each result.errors as err}
										<div class="error-item">
											<div class="error-item-header">
												<span class="error-item-param">{err.parameter}</span>
												<span class="error-item-method">{err.method}</span>
											</div>
											<code class="error-item-payload">{err.payload}</code>
											<span class="error-item-msg">{err.error}</span>
										</div>
									{/each}
								</div>
							{:else}
								<div class="empty-state">{$tr('commandInjection.noErrors')}</div>
							{/if}
						{/if}

						{#if activeResultTab === 'ostype'}
							{#if result.os_type_distribution.length > 0}
								<div class="dbtype-list">
									{#each result.os_type_distribution as dist}
										<div class="dbtype-card">
											<div class="dbtype-header">
												<span class="dbtype-name" style="color: {getOsTypeColor(dist.os_type)}">{getOsTypeIcon(dist.os_type)} {dist.os_type}</span>
												<span class="dbtype-count">{dist.count} {$tr('commandInjection.vulnsFound')}</span>
											</div>
											<div class="dbtype-bar-track">
												<div class="dbtype-bar-fill" style="width: {dist.count > 0 ? (dist.vulnerable_count / dist.count) * 100 : 0}%; background: {getOsTypeColor(dist.os_type)}"></div>
											</div>
											<div class="dbtype-meta">
												<span class="dbtype-vuln">{$tr('commandInjection.vulnerable')}: {dist.vulnerable_count}</span>
											</div>
										</div>
									{/each}
								</div>
							{:else}
								<div class="empty-state">{$tr('commandInjection.noOsData')}</div>
							{/if}
						{/if}
					{:else if processing}
						<div class="processing-state">
							<span class="processing-icon pulse">💉</span>
							<span class="processing-text">{$tr('commandInjection.scanning')}</span>
							<span class="processing-hint">{$tr('commandInjection.scanHint')}</span>
						</div>
					{:else}
						<div class="empty-state-main">
							<span class="empty-icon">💉</span>
							<span class="empty-text">{$tr('commandInjection.emptyHint')}</span>
						</div>
					{/if}
				</div>
			</div>
	...
	{:else if activeMainTab === 'history'}
		<div class="section-card"><ToolHistory toolType={TOOL_NAME} toolName={$tr('commandInjection.title')} bind:this={historyComponent} /></div>
...
<style>
	...
	.tab-btn {
		...
		color: #94a3b8; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 6px;
	}

	.tab-btn.active {
		background: linear-gradient(135deg, #a855f7 0%, #6366f1 100%);
		color: white; font-weight: 600; box-shadow: 0 2px 8px rgba(168, 85, 247, 0.3);
	}
	...
	.form-label {
		display: block; font-size: 0.75rem; color: #94a3b8; margin-bottom: 0.3rem;
	...
	.form-input {
		...
		color: #f1f5f9; font-size: 0.85rem; box-sizing: border-box; transition: border-color 0.2s;
	}
	...
	.mode-btn {
		padding: 0.4rem 0.5rem; border: 1px solid rgba(148, 163, 184, 0.15);
		border-radius: 0.4rem; background: rgba(15, 23, 42, 0.6); color: #94a3b8;
		cursor: pointer; font-size: 0.75rem; transition: all 0.2s; text-align: center;
	}
	...
	.target-chip {
		...
		border: 1px solid rgba(148, 163, 184, 0.15); border-radius: 0.4rem;
		background: rgba(15, 23, 42, 0.6); cursor: pointer; font-size: 0.75rem;
	...
	.btn-primary {
		flex: 1; background: linear-gradient(135deg, #a855f7 0%, #6366f1 100%);
		color: white; font-weight: 600; padding: 0.65rem 1.25rem; border: none;
	...
	.btn-secondary {
		background: rgba(148, 163, 184, 0.1); color: #94a3b8; padding: 0.65rem 1rem;
		border: 1px solid rgba(148, 163, 184, 0.15); border-radius: 0.5rem;
	...
	.error-card {
		...
		background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); border-radius: 0.5rem;
	}
	...
	.resource-score-badge {
		...
		border-radius: 0.5rem; border: 1px solid rgba(168, 85, 247, 0.3); background: rgba(168, 85, 247, 0.1);
	}
	...
	.export-select {
		padding: 0.35rem 0.5rem; border-radius: 0.4rem; border: 1px solid rgba(148, 163, 184, 0.15);
		background: rgba(15, 23, 42, 0.6); color: #94a3b8; font-size: 0.75rem;
	}

	.btn-export {
		padding: 0.35rem 0.75rem; border-radius: 0.4rem; border: 1px solid rgba(168, 85, 247, 0.3);
		background: rgba(168, 85, 247, 0.1); color: #c4b5fd; cursor: pointer; font-size: 0.8rem; transition: all 0.2s;
	}
	...
	.summary-bar {
		...
		background: rgba(15, 23, 42, 0.4); border-radius: 0.4rem; margin-bottom: 1rem;
	...
	.result-tab {
		padding: 0.4rem 0.75rem; border-radius: 0.4rem; border: 1px solid rgba(148, 163, 184, 0.15);
		background: rgba(15, 23, 42, 0.4); color: #94a3b8; cursor: pointer; font-size: 0.8rem; transition: all 0.2s;
	}
	...
	.overview-stat {
		...
		background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(148, 163, 184, 0.08); border-radius: 0.5rem;
	}
	...
	.chart-label { width: 100px; font-size: 0.75rem; color: #94a3b8; text-align: right; flex-shrink: 0; }
	...
	.param-chip {
		padding: 0.25rem 0.5rem; border-radius: 0.3rem; background: rgba(168, 85, 247, 0.1);
		border: 1px solid rgba(168, 85, 247, 0.2); color: #c4b5fd; font-size: 0.7rem; font-family: monospace;
	}

	.filter-bar { display: flex; gap: 0.5rem; margin-bottom: 1rem; flex-wrap: wrap; }

	.filter-input {
		flex: 1; min-width: 150px; padding: 0.4rem 0.6rem; border-radius: 0.4rem;
		border: 1px solid rgba(148, 163, 184, 0.15); background: rgba(15, 23, 42, 0.6);
		color: #f1f5f9; font-size: 0.8rem;
	}

	.filter-input:focus { outline: none; border-color: #a855f7; }

	.filter-select {
		padding: 0.4rem 0.5rem; border-radius: 0.4rem; border: 1px solid rgba(148, 163, 184, 0.15);
		background: rgba(15, 23, 42, 0.6); color: #94a3b8; font-size: 0.75rem;
	}
	...
	.vuln-card {
		...
		border-radius: 0.5rem; padding: 0.75rem; transition: border-color 0.2s;
	}
	...
	.vuln-header-left { display: flex; align-items: center; gap: 0.4rem; }

	.severity-badge { padding: 0.15rem 0.5rem; border-radius: 0.3rem; font-size: 0.7rem; font-weight: 700; }

	.method-badge {
		padding: 0.15rem 0.4rem; border-radius: 0.3rem; background: rgba(148, 163, 184, 0.1);
		color: #94a3b8; font-size: 0.7rem;
	}

	.type-badge { padding: 0.15rem 0.4rem; border-radius: 0.3rem; font-size: 0.7rem; }
	.os-badge { padding: 0.15rem 0.4rem; border-radius: 0.3rem; font-size: 0.7rem; }

	.confidence-badge { font-size: 0.75rem; color: #a855f7; font-weight: 600; }

	.vuln-body { display: flex; flex-direction: column; gap: 0.35rem; }
	.vuln-field { display: flex; align-items: baseline; gap: 0.5rem; }
	.field-label { font-size: 0.7rem; color: #64748b; min-width: 100px; flex-shrink: 0; }

	.field-value {
		font-size: 0.8rem; color: #e2e8f0; background: rgba(15, 23, 42, 0.6);
		padding: 0.15rem 0.4rem; border-radius: 0.25rem; word-break: break-all;
	}

	.payload-code { color: #fbbf24; border: 1px solid rgba(251, 191, 36, 0.2); }

	.vuln-meta { display: flex; gap: 0.4rem; margin-top: 0.25rem; flex-wrap: wrap; }
	...
	.safe-item {
		...
		background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(34, 197, 94, 0.1); border-radius: 0.4rem;
	}

	.safe-check { font-size: 0.85rem; }
	.safe-param { font-family: monospace; color: #e2e8f0; font-size: 0.8rem; flex: 1; }
	.safe-method { font-size: 0.7rem; color: #94a3b8; }
	.safe-tests { font-size: 0.7rem; color: #64748b; }

	.error-list { display: flex; flex-direction: column; gap: 0.5rem; }

	.error-item {
		padding: 0.6rem 0.75rem; background: rgba(239, 68, 68, 0.05);
		border: 1px solid rgba(239, 68, 68, 0.1); border-radius: 0.4rem;
	}

	.error-item-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.3rem; }
	.error-item-param { font-size: 0.8rem; color: #fca5a5; font-weight: 600; }
	.error-item-method { font-size: 0.7rem; color: #94a3b8; }
	.error-item-payload { display: block; font-size: 0.75rem; color: #fbbf24; margin-bottom: 0.25rem; word-break: break-all; }
	.error-item-msg { font-size: 0.75rem; color: #94a3b8; }

	.dbtype-list { display: flex; flex-direction: column; gap: 0.75rem; }

	.dbtype-card {
		padding: 0.75rem; background: rgba(15, 23, 42, 0.4);
		border: 1px solid rgba(148, 163, 184, 0.08); border-radius: 0.5rem;
	}

	.dbtype-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; }
	.dbtype-name { font-size: 0.9rem; font-weight: 600; }
	.dbtype-count { font-size: 0.75rem; color: #94a3b8; }

	.dbtype-bar-track { height: 8px; background: rgba(15, 23, 42, 0.4); border-radius: 4px; overflow: hidden; margin-bottom: 0.35rem; }
	.dbtype-bar-fill { height: 100%; border-radius: 4px; transition: width 0.5s ease; min-width: 4px; }
	.dbtype-meta { display: flex; gap: 0.5rem; }
	.dbtype-vuln { font-size: 0.7rem; color: #fca5a5; }
