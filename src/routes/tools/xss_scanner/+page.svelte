<script lang="ts">
	...
	interface XssSafeEntry {
		parameter: string;
		tests_run: number;
		method: string;
	}

	interface XssErrorEntry {
		parameter: string;
		payload: string;
		error: string;
		method: string;
	...
	interface XssScanResult {
		...
		safe_parameters: XssSafeEntry[];
		errors: XssErrorEntry[];
		tests_performed: number;
		parameters_tested: string[];
		scan_duration_ms: number;
		summary: string;
		xss_type_distribution: XssTypeDistribution[];
	}
	...
	let result: XssScanResult | null = $state(null);
	let error = $state('');
	...
	const TOOL_NAME = 'xss_scanner';
	let historyComponent = $state<ToolHistory>();

	function getScanLevelLabel(level: string): string {
		const labels: Record<string, string> = {
			basic: $tr('xssScanner.levelBasic'),
			moderate: $tr('xssScanner.levelModerate'),
			aggressive: $tr('xssScanner.levelAggressive')
		};
	...
	function getXssTypeColor(xssType: string): string {
		switch (xssType) {
			case 'reflected': return '#3b82f6';
			case 'dom': return '#a855f7';
			case 'stored': return '#ef4444';
			default: return '#64748b';
		}
	}
	...
	function getFilteredVulns() {
		return result?.vulnerabilities.filter(v => {
			...
			if (typeFilter !== 'all' && v.xss_type !== typeFilter) return false;
			if (contextFilter !== 'all' && v.injection_context !== contextFilter) return false;
			...
			if (searchQuery) {
				...
				return v.parameter.toLowerCase().includes(q) ||
					...
					v.xss_type.toLowerCase().includes(q);
			}
			return true;
		}) || [];
	}
	...
	function getUniqueTypes() {
		return [...new Set(result?.vulnerabilities.map(v => v.xss_type) || [])];
	}

	function getUniqueContexts() {
		return [...new Set(result?.vulnerabilities.map(v => v.injection_context) || [])];
	}
	...
	function getTypeChartData() {
		...
		const maxVulns = Math.max(...types.map(t => result!.vulnerabilities.filter(v => v.xss_type === t).length));
		return types.map(t => ({
			xss_type: t,
			count: result!.vulnerabilities.filter(v => v.xss_type === t).length,
			percent: maxVulns > 0 ? (result!.vulnerabilities.filter(v => v.xss_type === t).length / maxVulns) * 100 : 0
		}));
	}

	async function scan() {
		if (!url.trim()) {
			error = $tr('xssScanner.urlRequired');
			return;
		}
		if (!testGet && !testPost && !testCookies && !testHeaders) {
			error = $tr('xssScanner.methodRequired');
			return;
		}
		...
		try {
			...
			result = await invoke<XssScanResult>('scan_xss_command', {
				config: {
					url: url.trim(),
					timeout,
					threads,
	...
	async function exportResult() {
		...
		try {
			...
			const path = await save({
				defaultPath: `xss_scan_${Date.now()}.${exportFormat}`,
			...
			if (exportFormat === 'json') {
				...
				const headers = ['Parameter', 'XSS Type', 'Context', 'Severity', 'Payload', 'Evidence', 'Confidence', 'Method', 'Status', 'Response Time'];
				const rows = result.vulnerabilities.map(v =>
					[v.parameter, v.xss_type, v.injection_context, v.severity, v.payload, v.evidence, v.confidence.toString(), v.method, (v.http_status || '').toString(), (v.response_time_ms || '').toString()]
				);
...
<div class="nd-page">
		...
		<div class="content-grid">
			<div class="input-section">
				<div class="section-card">
					<h2 class="section-title">{$tr('xssScanner.configTitle')}</h2>
					<p class="section-desc">{$tr('xssScanner.configDesc')}</p>

					<div class="form-group">
						<label class="form-label">{$tr('xssScanner.targetUrl')}</label>
					...
					<div class="form-group">
						<label class="form-label">{$tr('xssScanner.scanLevel')}</label>
						<div class="mode-grid">
					...
					<div class="form-group">
						<label class="form-label">🎯 {$tr('xssScanner.customParams')}</label>
					...
					<div class="button-group">
						<button class="btn-primary" onclick={scan} disabled={processing || !url.trim() || (!testGet && !testPost && !testCookies && !testHeaders)}>
							{#if processing}<span class="spinner"></span>{$tr('xssScanner.scanning')}{:else}💥 {$tr('xssScanner.startScan')}{/if}
						</button>
					...
					{:else if result}
						<div class="result-header">
							...
							<div class="header-actions">
								<div class="resource-score-badge">
									...
									<span class="score-label">{$tr('xssScanner.vulnsFound')}</span>
								</div>
						...
						<div class="result-tabs">
							<button class="result-tab {activeResultTab === 'overview' ? 'active' : ''}" onclick={() => activeResultTab = 'overview'}>
								📊 {$tr('xssScanner.tabOverview')}
							</button>
							<button class="result-tab {activeResultTab === 'vulns' ? 'active' : ''}" onclick={() => activeResultTab = 'vulns'}>
								🔴 {$tr('xssScanner.tabVulns')} ({result.vulnerabilities.length})
							</button>
							<button class="result-tab {activeResultTab === 'safe' ? 'active' : ''}" onclick={() => activeResultTab = 'safe'}>
								🟢 {$tr('xssScanner.tabSafe')} ({result.safe_parameters.length})
							</button>
							{#if result.errors.length > 0}
								<button class="result-tab {activeResultTab === 'errors' ? 'active' : ''}" onclick={() => activeResultTab = 'errors'}>
									⚠️ {$tr('xssScanner.tabErrors')} ({result.errors.length})
								</button>
							{/if}
							{#if result.xss_type_distribution.length > 0}
								<button class="result-tab {activeResultTab === 'xsstype' ? 'active' : ''}" onclick={() => activeResultTab = 'xsstype'}>
									📊 {$tr('xssScanner.tabXssType')}
								</button>
							{/if}
						</div>
						...
						{#if activeResultTab === 'overview'}
							<div class="overview-grid">
								<div class="overview-stat">
									<span class="stat-label">{$tr('xssScanner.testsPerformed')}</span>
									<span class="stat-value">{result.tests_performed}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">🔴 {$tr('xssScanner.highSeverity')}</span>
								...
								<div class="overview-stat">
									<span class="stat-label">🟡 {$tr('xssScanner.mediumSeverity')}</span>
								...
								<div class="overview-stat">
									<span class="stat-label">🟢 {$tr('xssScanner.lowSeverity')}</span>
								...
								<div class="overview-stat">
									<span class="stat-label">🎯 {$tr('xssScanner.paramsTested')}</span>
									<span class="stat-value">{result.parameters_tested.length}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">🟢 {$tr('xssScanner.safeParams')}</span>
									<span class="stat-value" style="color: #22c55e">{result.safe_parameters.length}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">⏱️ {$tr('xssScanner.scanDuration')}</span>
									<span class="stat-value">{formatDuration(result.scan_duration_ms)}</span>
								</div>
								<div class="overview-stat">
									<span class="stat-label">⚠️ {$tr('xssScanner.errorCount')}</span>
							...
							{#if result && result.vulnerabilities.length > 0}
								<div class="category-chart">
									<h3 class="subsection-title">{$tr('xssScanner.xssTypeChart')}</h3>
									<div class="chart-bars">
										{#each getTypeChartData() as data}
											<div class="chart-row">
												<span class="chart-label">{data.xss_type}</span>
												<div class="chart-bar-track">
													<div class="chart-bar-fill" style="width: {data.percent}%; background: {getXssTypeColor(data.xss_type)}"></div>
												</div>
							...
							{#if result && result.parameters_tested.length > 0}
								<div class="params-list">
									<h3 class="subsection-title">{$tr('xssScanner.testedParamsList')}</h3>
									<div class="chip-list">
						...
						{#if activeResultTab === 'vulns'}
							{#if result && result.vulnerabilities.length > 0}
								<div class="filter-bar">
									<input type="text" bind:value={searchQuery} placeholder="{$tr('xssScanner.searchVulns')}..." class="filter-input" />
									<select bind:value={severityFilter} class="filter-select">
										<option value="all">{$tr('xssScanner.allSeverities')}</option>
										<option value="high">🔴 High</option>
										<option value="medium">🟡 Medium</option>
										<option value="low">🟢 Low</option>
									</select>
									<select bind:value={typeFilter} class="filter-select">
										<option value="all">{$tr('xssScanner.allTypes')}</option>
										{#each getUniqueTypes() as t}
											<option value={t}>{t}</option>
										{/each}
									</select>
									<select bind:value={contextFilter} class="filter-select">
										<option value="all">{$tr('xssScanner.allContexts')}</option>
										{#each getUniqueContexts() as c}
											<option value={c}>{c}</option>
										{/each}
									</select>
									<select bind:value={methodFilter} class="filter-select">
										<option value="all">{$tr('xssScanner.allMethods')}</option>
										...
										<div class="vuln-card">
											<div class="vuln-header">
												<div class="vuln-header-left">
													...
													<span class="type-badge" style="background: {getXssTypeColor(vuln.xss_type)}20; color: {getXssTypeColor(vuln.xss_type)}">{vuln.xss_type}</span>
													<span class="context-badge">{getContextIcon(vuln.injection_context)} {vuln.injection_context}</span>
												</div>
												<span class="confidence-badge">
													{$tr('xssScanner.confidence')}: {getConfidencePercent(vuln.confidence)}%
												</span>
											</div>
											<div class="vuln-body">
												<div class="vuln-field">
													<span class="field-label">🎯 {$tr('xssScanner.parameter')}</span>
							...
							{:else}
								<div class="safe-state">
									...
									<span class="safe-text">{$tr('xssScanner.noVulns')}</span>
								</div>
							{/if}
						{/if}
								...
								<div class="safe-list">
									{#each result.safe_parameters as safe}
										<div class="safe-item">
											...
											<span class="safe-tests">{$tr('xssScanner.testsRun')}: {safe.tests_run}</span>
										</div>
									{/each}
								</div>
							{:else}
								<div class="empty-state">{$tr('xssScanner.noSafeParams')}</div>
							{/if}
						{/if}
						...
						{#if activeResultTab === 'errors'}
							...
							{:else}
								<div class="empty-state">{$tr('xssScanner.noErrors')}</div>
							{/if}
						{/if}

						{#if activeResultTab === 'xsstype'}
							{#if result.xss_type_distribution.length > 0}
								<div class="dbtype-list">
									{#each result.xss_type_distribution as dist}
										<div class="dbtype-card">
											<div class="dbtype-header">
												<span class="dbtype-name" style="color: {getXssTypeColor(dist.xss_type)}">{dist.xss_type}</span>
												<span class="dbtype-count">{dist.count} {$tr('xssScanner.vulnsFound')}</span>
											</div>
											<div class="dbtype-bar-track">
												<div class="dbtype-bar-fill" style="width: {dist.count > 0 ? (dist.vulnerable_count / dist.count) * 100 : 0}%; background: {getXssTypeColor(dist.xss_type)}"></div>
											</div>
											<div class="dbtype-meta">
												<span class="dbtype-vuln">{$tr('xssScanner.vulnerable')}: {dist.vulnerable_count}</span>
											</div>
										</div>
									{/each}
								</div>
							{:else}
								<div class="empty-state">{$tr('xssScanner.noTypeData')}</div>
							{/if}
						{/if}
					{:else if processing}
						<div class="processing-state">
							<span class="processing-icon pulse">💥</span>
							<span class="processing-text">{$tr('xssScanner.scanning')}</span>
							<span class="processing-hint">{$tr('xssScanner.scanHint')}</span>
						</div>
					{:else}
						<div class="empty-state-main">
							<span class="empty-icon">💥</span>
							<span class="empty-text">{$tr('xssScanner.emptyHint')}</span>
						</div>
					{/if}
				</div>
			</div>
	...
	{:else if activeMainTab === 'history'}
		<div class="section-card"><ToolHistory toolType={TOOL_NAME} toolName={$tr('xssScanner.title')} bind:this={historyComponent} /></div>
...
<style>
	...
	.context-badge {
		padding: 0.15rem 0.4rem; border-radius: 0.3rem; background: rgba(34, 197, 94, 0.15);
		color: #86efac; font-size: 0.7rem;
	}
